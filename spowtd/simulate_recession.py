...
def dump_simulated_recession(connection, parameter_file, outfile, observations_only):
    ...
    """
    (avg_elapsed_time_d, avg_zeta_cm, elapsed_time_d) = simulate_recession(
        connection, parameter_file
    )

    if observations_only:
        outfile.write('# Recession curve simulation vector\n')
        yaml.dump(list(reversed(elapsed_time_d.tolist())), outfile)
    else:
        yaml.dump(
            (
                [
...
def simulate_recession(connection, parameter_file):
    ...
    return (avg_elapsed_time_d, avg_zeta_cm, elapsed_time_d)


def compute_recession_curve(
    specific_yield,
    transmissivity_m2_d,
    zeta_grid_mm,
    mean_elapsed_time_d,
