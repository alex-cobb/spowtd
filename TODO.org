 ...
 - [ ] Factor out shared code from get_series_time_offsets and call it
   ...
   storage offsets)
 - [ ] Add a new function for the rise curve, get_series_storage_offsets, that
   just calls get_series_offsets.
 - [ ] In rise.py, call get_series_storage_offsets
 - [ ] Add a flag in rise.py::find_rise_offsets ->
   rise.py::compute_rise_offsets -> fit_offsets.py::get_series_storage_offsets
   to compute and use the covariance matrix
   - The "series" argument passed in to the call to get_series_storage_offsets
     contains sufficient information to compute the covariance matrix
   - Put in stub code to raise NotImplementedError if this flag is passed
 - [ ] Add a function to compute the covariance matrix for rise analysis
 - */Refactoring for rise analysis*
