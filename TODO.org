 ...
 - [X] Add a function in rise.py to compute the covariance matrix for
   rise analysis
 - [X] Add a flag in rise.py::find_rise_offsets ->
   rise.py::compute_rise_offsets -> fit_offsets.py::get_series_storage_offsets
   to use the covariance matrix
 - [X] Create a function that assembles the covariance matrix for
 ...
 - [X] Add optional weighting of linear system so each event is
   weighted equally
