...
cdef class Interpolant(ScalarFunction):
    ...
    A local copy of the data in the input vectors is held by the interpolant.
    ...
    def __cinit__(self,
        ...
        # Disable the default GSL error handler (core dump) and never restore; as
        # recommended in the library, we check return codes.
    ...
    cpdef update(self):
        ...
        The update does not verify that x and y are still finite and that x is strictly
        ascending.  If those assumptions are violated, behaviour is undefined.
    ...
    def __call__(self, double x):
        ...
        For calls from C, use o.call() instead.  This method exists primarily for
        testing from Python.
...
def spline_to_tolerance(function, double xa, double xb,
    ...
    Splines the function with a sufficient number of knots that root mean squared error
    is no greater than max_rmse.  Starting with four points, the spline is created, and
    the root-mean-squared error computed on the interval xa, xb by quadrature; if the
    rmse constraint is not satisfied, the number of points is doubled and the procedure
    is repeated.
    ...
    while not rmse <= max_rmse:
        ...
        interpolant = Interpolant(knots,
                                  np.array([function(knot)
                                            for knot in knots]),
                                  degree=3)
        # full_output=1 suppresses warnings about reaching the maximum number of
        # subdivisions of an interval
